...
public:
    ...
    */
    ...
    virtual bool FilterNounNumeral(std::string &gcNoun, const std::string &gcNum, grammems_mask_t &grammems) const {
        assert(false);
        return false;
    };

    virtual grammems_mask_t
    ChangeGleicheAncode1(GrammemCompare, const std::string &, std::string &, const grammems_mask_t) const {
        assert(false);
        return 0;
    };
